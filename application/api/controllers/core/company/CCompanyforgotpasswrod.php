...
class CCompanyforgotpasswrod extends CAaskController {
    ...
    public function execute() {
        ...
        try {
            ...
            if (isset($request) && !empty($request)) {
                $postdata = $_POST;// json_decode($request, true);
                $sql = $this->ask_mysqli->select("company", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("email" => $postdata["userName"]));
        ...
        try {
            if ($result->num_rows > 0) {
                if ($row = $result->fetch_assoc()) {
                    $name = $row["user"];
                    $email = $row["email"];
                    $passwordResetLink = companyurl . "?r=reset&e=" . $this->encript->encTxt($email) . "&i=" . $this->encript->encTxt($row["id"]);
    ...
    function createLink($passLink) {
        ...
        return companyurl . "?r=short&p=". $this->encript->encTxt($id);
    }
