...
class collegeLogin extends CAaskController {
    ...
    function checkLoginUser($postData) {
        $data = null;
        try {
            $sql = $this->ask_mysqli->select("college", $_SESSION["db_1"]) . $this->ask_mysqli->where(array("email" => $postData["userName"],"isActive"=>1),"AND");
            $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
            if ($row = $result->fetch_assoc()) {
                if (password_verify($postData["password"], $row["pwd"])) {
                    $data = array("message" => "success", "_id" => $row["id"], "email" => $row["email"],"row"=>$row);
                } else {
                    $data = null;
                }
            } else {
