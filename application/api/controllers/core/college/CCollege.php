...
class CCollege extends CAaskController {
    ...
    function save() {
        ...
        unset($data["id"]);
        if (isset($_FILES)) {
            $fileData = $this->uploadFiletoFileSystem('logo',"assets/upload/profile");
        }
        ...
        if (empty($error)) {
            ...
            echo json_encode(array("toast" => array("success", "college", " Added Successfully"), "status" => 1, "message" => "Insert Successfully.. Check Email and ge password for login", "data" => $data));
        } else {
            // $this->sendMailtoUser($_POST["college"], $_POST["email"], $password);
            echo json_encode(array("toast" => array("danger", "college", " Failed to add " . $error[0]), "status" => 0, "message" => $this->adminDB[$_SESSION["db_1"]]->error . " Insert failed " . $error[0]));
    ...
    function update() {
        ...
        $sql = $this->ask_mysqli->update($data, "college") . $this->ask_mysqli->whereSingle($where);
        $this->adminDB[$_SESSION["db_1"]]->autocommit(false);
        $error = array();
        $this->adminDB[$_SESSION["db_1"]]->query($sql) == true ? true : array_push($error, "inset Error " . $this->adminDB[$_SESSION["db_1"]]->error);
        if (empty($error)) {
    ...
    function select() {
        $where = array("id" => $_POST["id"]);
        $sql = $this->ask_mysqli->select("college", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle($where);
    ...
    function delete() {
        $where = array("id" => $_POST["id"]);
        $sql = $this->ask_mysqli->delete("college") . $this->ask_mysqli->whereSingle($where);
    ...
    function sendMailtoUser($name, $email, $password) {
        ...
        $data = array(
            ...
            "mail" => $this->mailObject->sendmailWithoutAttachment($email, noreplayid, company, $content, "College account create successfully", "")
        );
    ...
    function unilist() {
        try {
            $data = array();
            $restult = $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->select("university", $_SESSION["db_1"]));
