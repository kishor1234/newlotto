...
class CollegeData extends CAaskController {
        ...
        try {
            switch ($_POST['action']) {
                case "getCollegeProfile":
                    $this->getCollegeProfile();
                    break;
                case "collegeUploadProfilePhoto":
                    $this->collegeUploadProfilePhoto();
                    break;
            }
        } catch (Exception $ex) {
            echo $ex->getMessage();
    ...
    function collegeUploadProfilePhoto() {
...
//        $imageid = $this->adminDB[$_SESSION["db_1"]]->insert_id;
        ...
        $sql = $this->ask_mysqli->update($data, "college") . $this->ask_mysqli->where($where, "AND");
    ...
    function getCollegeProfile() {
        $data = array();
        $sql = $this->ask_mysqli->select("college", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("id" => $_POST['id']));
