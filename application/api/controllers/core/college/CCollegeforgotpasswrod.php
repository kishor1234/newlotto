...
class CCollegeforgotpasswrod extends CAaskController {
    ...
    public function execute() {
        ...
        try {
            ...
            if (isset($request) && !empty($request)) {
                ...
                $postdata = $request;
                $sql = $this->ask_mysqli->select("college", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("email" => $postdata["userName"]));
        ...
        try {
            if ($result->num_rows > 0) {
                if ($row = $result->fetch_assoc()) {
                    $name = $row["name"];
                    $email = $row["email"];
                    $passwordResetLink = collegeurl . "?r=reset&e=" . $this->encript->encTxt($email) . "&i=" . $this->encript->encTxt($row["id"]);
    ...
    function createLink($passLink) {
        ...
        return collegeurl . "?r=short&p=". $this->encript->encTxt($id);
    }
