...
class CCollegeChangePassword extends CAaskController {
    ...
    function changePassword($postData) {
        $data = null;
        try {
            $sql = $this->ask_mysqli->select("college", $_SESSION["db_1"]) . $this->ask_mysqli->where(array("email" => $this->encript->decTxt($postData["etoken"]), "id" => $this->encript->decTxt($postData["token"])), "AND");
            $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
            if ($row = $result->fetch_assoc()) {
                if (strcmp($row["email"], $this->encript->decTxt($postData["etoken"])) == 0) {
                    $sql = $this->ask_mysqli->update(array("pwd" => password_hash($postData["pass"], PASSWORD_DEFAULT)), "college") . $this->ask_mysqli->where(array("email" => $this->encript->decTxt($postData["etoken"]), "id" => $this->encript->decTxt($postData["token"])), "AND");
