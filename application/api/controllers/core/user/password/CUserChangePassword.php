        ...
        try {
            ...
            if ($row = $result->fetch_assoc()) {
                if (strcmp($row["email"], $this->encript->decTxt($postData["etoken"])) == 0) {
                    $sql = $this->ask_mysqli->update(array("pwd" => password_hash($postData["pass"], PASSWORD_DEFAULT)), "user") . $this->ask_mysqli->where(array("email" => $this->encript->decTxt($postData["etoken"]), "id" => $this->encript->decTxt($postData["token"])), "AND");
