...
class userAddPersonalInfo extends CAaskController {
    ...
    public function execute() {
        ...
        switch ($_POST["action"]) {
            case "add":
                ...
                $sql = $this->ask_mysqli->insert("userpersonal", $data);
                $error = array();
                $this->adminDB[$_SESSION["db_1"]]->query($sql) == true ? true : array_push($error, "Insert Error on user personal");
                $sql = $this->ask_mysqli->update(array("personal" => 1), "user") . $this->ask_mysqli->where($where, "AND");
                ...
                if (empty($error)) {
                    ...
                    echo json_encode(array("status" => 1, "message" => "Personal information add successfully"));
                } else {
                    $this->adminDB[$_SESSION["db_1"]]->rollback();
                    echo json_encode(array("status" => 0, "message" => "Personal information add Failded"));
                }
                break;
            case "update":
                break;
