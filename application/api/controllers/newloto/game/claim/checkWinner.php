                ...
                if ($claimStatis === "1") {
                    ...
                    if (empty($winArray)) {
                       ...
                       // $er == false ? array_push($error, "Update win and claim status  table error " . $this->adminDB[$_SESSION["db_1"]]->error) : true;
                        ...
                        $this->amount = $sum * 2;
                        $error = array();
                        $er = $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->insert("claim", array("enteryid" => $_POST["userid"], "winnumber" => json_encode($winArray), "gameid" => $row["gametimeid"], "gametime" => $row["gametime"], "gameetime" => $row["gameendtime"], "cdate" => $row["enterydate"])));
